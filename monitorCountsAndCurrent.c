...
int main (int argc, char **argv)
{
	...
	if (argc == 5){
		dwell= atoi(argv[1]);
		totalMeasurements= atoi(argv[2]);
		magnitude= atoi(argv[3]);
		strcpy(comments,argv[4]);
	} else{
		printf("    Usage:                                                                                     \n");
		printf("           sudo ./monitorCountsAndCurrent <time per measurement> <total measurements> <orderOfMagnitudeOfCurrent> <comments>\n");
		printf("                                                                                               \n");
		return 1;
	}
